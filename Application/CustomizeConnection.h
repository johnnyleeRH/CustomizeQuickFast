...
# pragma once
#endif
#ifndef CUSTOMIZECONNECTION_H
#define CUSTOMIZECONNECTION_H
...
#include <Communication/CustomizeRawReceiver.h>
...
namespace QuickFAST{
  namespace Application{
    ...
    ///
    /// Each source of FAST encoded data should be supported by a separate instance of CustomizeConnection.
    /// Examples of sources include a single multicast group; a FAST encoded data file; etc.
    class QuickFAST_Export CustomizeConnection
    {
    public:
      CustomizeConnection();
      ~CustomizeConnection();
      ...
      /// @param configuration contains configuration parametergs
      void configure(Messages::ValueMessageBuilder & builder, Application::DecoderConfiguration &configuration);

      void addReceiveBuf(const unsigned char *buf, size_t bufferSize);

      bool decodefinished()
      {
        return receiver_->decodefinished();
      }
      //////////////////////////////////////////////
      ...
      Codecs::TemplateRegistryPtr & registry()
      {
        if(!registry_)
        {
          throw UsageError("Coding Error","Using CustomizeConnection registry before it is configured.");
        }
        return registry_;
      }
      ...
      Codecs::HeaderAnalyzer & messageHeaderAnalyzer() const
      {
        ...
        {
          throw UsageError("Coding Error","Using CustomizeConnection message header analyzer before it is configured.");
        }
      ...
      Communication::Assembler & assembler()const
      {
        if(!assembler_)
        {
          throw UsageError("Coding Error","Using CustomizeConnection assembler before it is configured.");
        }
        return *assembler_;
      }
      ...
      Communication::Receiver & receiver()const
      {
        if(!receiver_)
        {
          throw UsageError("Coding Error","Using CustomizeConnection receiver before it is configured.");
        }
        return *receiver_;
      }
    ...
    private:
      ...
      boost::scoped_ptr<Communication::CustomizeRawReceiver> receiver_;

    };
  }
}

#endif // CUSTOMIZECONNECTION_H
